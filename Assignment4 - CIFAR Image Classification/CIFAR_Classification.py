...
# model.compile).
import tensorflow
from keras.layers import  Conv2D, Dense, Input, MaxPooling2D, Dropout, Flatten, Softmax, BatchNormalization
from keras import activations #ativations.relu
...
def myGetModel(data):
    ...
    CNN.add(Conv2D(filters, (kernel,kernel), padding='same',activation='relu', kernel_regularizer=regularizers.l2(weight)))
    CNN.add(BatchNormalization())
    CNN.add(MaxPooling2D(pool_size=(2,2)))
    CNN.add(Dropout(0.2))
    

    CNN.add(Conv2D(2*filters, (kernel,kernel), padding='same',activation='relu', kernel_regularizer=regularizers.l2(weight)))
    CNN.add(BatchNormalization())
    CNN.add(MaxPooling2D(pool_size=(2,2)))
    CNN.add(Dropout(0.3))
    
    CNN.add(Conv2D(4*filters, (kernel,kernel), padding='same',activation='relu', kernel_regularizer=regularizers.l2(weight)))
    CNN.add(BatchNormalization())
    CNN.add(MaxPooling2D(pool_size=(2,2)))
    CNN.add(Dropout(0.4))
...
def myFitModel(CNN,data):
    epochs = 300
    Bsize = 1000
    earlystop= EarlyStopping(monitor='val_loss',patience=20, verbose=2,restore_best_weights=True)
    checkpoint = ModelCheckpoint(filepath='Checkpoint/',monitor='val_loss', verbose=2)
    History = CNN.fit(data.x_train, data.y_train, verbose=2, epochs=epochs,batch_size=Bsize,
                shuffle=True, callbacks=[earlystop,checkpoint], validation_data=(data.x_valid, data.y_valid))
    #History = CNN.fit(data.x_train, data.y_train, verbose=2, epochs=epochs)
    return CNN
...
def runImageClassification(getModel=None,fitModel=None,seed=7):
    ...
    # Fit model
    print("Fitting model...")
    CNN=fitModel(CNN,data)

    # Evaluate on test data
    print("Evaluating model...")
    score = CNN.evaluate(data.x_test, data.y_test, verbose=2)
...
if __name__ == '__main__':
    runImageClassification(getModel=myGetModel,fitModel=myFitModel,seed=7)